...
{
    public class CableTvCompanyMain
    {
        ...
        //which points are connected by those paths. But the cost of some of the paths is more expensive because they 
        //are longer. If every house is a node and every path from house to house is an edge, find a way to minimize 
        //the cost for cables.
        ...
        public static void Main()
        {
            ...
            {
                if (edge.FromCity == edges[0].FromCity)
                {
                    priority.Add(edge);
                }
            }

            used[edges[0].FromCity] = true;
        ...
        private static void FindMinimumSpanningTree(bool[] used, SortedSet<Edge> priority, List<Edge> mpdEdges, List<Edge> edges)
        {
            while (priority.Count > 0)
            {
                ...
                if (!used[edge.ToCity])
                {
                    used[edge.ToCity] = true; // we "visit" this node
                    mpdEdges.Add(edge);
        ...
        private static void AddEdges(Edge edge, List<Edge> edges, List<Edge> mpd, SortedSet<Edge> priority, bool[] used)
        {
            for (int i = 0; i < edges.Count; i++)
            {
                ...
                {
                    if (edge.ToCity == edges[i].FromCity && !used[edges[i].ToCity])
                    {
